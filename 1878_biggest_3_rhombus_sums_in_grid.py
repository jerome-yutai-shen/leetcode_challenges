# -*- coding: utf-8 -*-
"""
Created on Sep 16 18:02:16 2023
        ...
        for k in range((min(len(grid), len(grid[0])) + 1) // 2):
            for i in range(k, len(grid) - k):
                for j in range(k, len(grid[0]) - k):
                    total = (((left[i][j - k] - left[i - k][j]) + (right[i][j + k] - right[i - k][j]) + grid[i - k][
                        j]) +
                             ((left[i + k][j] - left[i][j + k]) + (right[i + k][j] - right[i][j - k]) - grid[i + k][
                                 j])) if k else grid[i][j]
                    if total in lookup:
                        continue
                    lookup.add(total)
